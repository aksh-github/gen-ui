...
export const loadUI = async (url: string) => {
  try {
...
export const validate = (uiJson: any, name: string, value: ANY) => {
  if (!uiJson || !uiJson.form || !uiJson.form.children) {
    console.error("UI JSON not loaded yet");
    return "";
  }

  const field = uiJson.form.children.find(
...
export const isFormValid = (uiJson: any, formState: any) => {
  if (!uiJson || !formState) {
